...
def multi_thd_reac(reac_lst_orig):
    ...
    judgment_mode = 0 # 0: G; 1: homo-lumo; 2: random; 3: G + homo-lumo
    lumo_r, homo_r = 0, 0
    if(judgment_mode == 0):
        if(g1+g2-g3>0): # A + B => C
            ret.append(1)
...
if __name__ == '__main__':
    ...
    logging.info('reac_cnt: ' + str(reac_cnt))
    logging.info('len(reac_lst): ' + str(len(reac_lst)))
    if(multi_thd):
        ...
        reac_lst = reac_lst_1
    import pickle
    with open('Reactant_reactions_gibbs.txt', 'wb') as fp:

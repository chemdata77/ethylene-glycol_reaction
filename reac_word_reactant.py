...
h=int(argv[1])
print(h)
with open('Reactant_reactions_gibbs_test_63.txt', 'rb') as fp: 
    q = pickle.load(fp)  

with open('Reactant_reactions_hl_test_63.txt', 'rb') as fp:
    hl = pickle.load(fp)

with open('Reactant_reactions_all_test_81.txt', 'rb') as fp:
    al = pickle.load(fp)
...
run1 = p1.add_run('乙二醇作为反应物的反应表'+str(math.ceil(len(q)/5000))+'-'+str(int(i/5000)+1))
run1.font.name = '宋体'
...
path_model = '乙二醇作为反应物的反应表'+str(math.ceil(len(q)/5000))+'-'+str(int(i/5000)+1)+'.docx'

for j in range(len(b)): 
    row_cells = table.add_row().cells
    for k in range(3):
        run=table.cell(j+1,k).paragraphs[0].add_run() 
        picture =run.add_picture("/data/tp/picture01/"+str(b[j][k])+".png")
        #picture =run.add_picture("/data/tp/picture_out/"+str(b[i][j])+".png")
        picture.height=Cm(2)
        picture.width=Cm(2)
    
        run4=table.cell(j+1,k).paragraphs[0].add_run('\nG:\n'+str(('%.5f' % b[j][4][k*2]))+'\nHOMO:\n'+str(('%.5f' % hl[j][4][k*2]))+'\nLUMO:\n'+str(('%.5f' % hl[j][4][k*2+1]))+'\n'+'factor:'+str(('%d' % b[j][4][k*2+1]))) 
    
    row_cells[3].text = str(b[j][3])+'\n'+'('+str(('%.5f' % b[j][4][6]))+')'
